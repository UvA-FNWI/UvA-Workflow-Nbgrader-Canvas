...
class Course:
    ...
    def visualize_grades(self, assignment_id, min_grade, max_score):
        ...
        grades_by_interval = pd.DataFrame(grades.groupby([pd.cut(grades, interval)]).size())
        ...
        grades_by_interval['color'] = grades_by_interval.apply(self.color_grades, axis=1)
    ...
    def create_rir(self, df):
        ...
        rir_df = pd.DataFrame.from_dict(
            rir_dict, orient='index', columns=["Rir-waarde"])
        ...
        for n, c in enumerate(l):
            ...
            for requirement in self.requirements:
                if isinstance(requirement["groups"], list):
                    ...
                    insuifficent += list(weighted_total[weighted_total
            ...
            list_of_categories.append([c, len(df) - len(set(insuifficent))] + [fail_counter[x]
    ...
    def final_grades(self):
        ...
        if set(self.canvas_and_nbgrader()).issuperset(set(
                itertools.chain.from_iterable(v["assignments"] for k, v in self.groups.items() if v["weight"]>0))):
